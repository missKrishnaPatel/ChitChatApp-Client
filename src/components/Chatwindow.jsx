      ...
      {selectedUser || selectedGroup ? (
        <>
          ...
          <div className="bg-white p-4 border-b">
            ...
            {selectedGroup && (
              <div className="flex gap-2 mt-2">
                <button
                  onClick={() => { setShowMembers((v) => !v); setShowAddUser(false); }}
                  className="text-sm px-3 py-1 bg-gray-100 rounded-full hover:bg-gray-200"
                >
                  Members
                </button>
                <button
                  onClick={() => { setShowAddUser((v) => !v); setShowMembers(false); }}
                  className="text-sm px-3 py-1 bg-gray-100 rounded-full hover:bg-gray-200"
                >
                  Add User
                </button>
            ...
            {selectedGroup && showMembers && (
              <div className="mt-3 border rounded-lg divide-y">
                {(selectedGroup.members ?? []).map((member) => (
                  <div key={member._id} className="flex items-center justify-between px-3 py-2 text-sm">
                    <span>{member.firstName} {member.lastName}</span>
                    <div className="flex gap-2">
                      <button
                        onClick={() => removeMember(selectedGroup._id, member._id)}
                      ...
                      {isGroupAdmin(selectedGroup, member._id) ? (
                        ...
                        <button
                          onClick={() => makeAdmin(selectedGroup._id, member._id)}
            ...
            {selectedGroup && showAddUser && (
              <div className="mt-3 border rounded-lg divide-y max-h-48 overflow-y-auto">
                {users
                  .filter((u) => !(selectedGroup.members ?? []).some((m) => String(m._id) === String(u._id)))
                  .map((user) => (
                    <div key={user._id} className="flex items-center justify-between px-3 py-2 text-sm">
                ...
                {users.filter((u) => !(selectedGroup.members ?? []).some((m) => m._id === u._id)).length === 0 && (
                  <p className="px-3 py-2 text-sm text-gray-400">No users to add</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto bg-gray-50 space-y-3">
            {groupMessagesLoading ? (
              <p className="text-center text-gray-400 mt-10">Loading group messages...</p>
            ) : messages.length > 0 ? (
              messages.map((msg, index) => {
                const currentUserId = JSON.parse(atob(token.split(".")[1])).userId;
                const isSender = getId(msg.senderId) === currentUserId;

                const messageDate = new Date(msg.createdAt || msg.timestamp).toLocaleDateString([], {
                  day: "numeric", month: "short", year: "numeric",
                });
                const messageTime = new Date(msg.createdAt || msg.timestamp).toLocaleTimeString([], {
                  hour: "2-digit", minute: "2-digit",
                });

                const previousMsg = messages[index - 1];
                const previousDate = previousMsg
                  ? new Date(previousMsg.createdAt || previousMsg.timestamp).toLocaleDateString([], {
                      day: "numeric", month: "short", year: "numeric",
                    })
                  : null;
                  ...
                  <div key={msg._id || index}>
                    ...
                    <div className={`flex ${isSender ? "justify-end" : "justify-start"}`}>
                      <div className="flex flex-col max-w-xs">
                        <p className={`text-xs mb-1 font-semibold ${isSender ? "text-right text-indigo-600" : "text-left text-gray-600"}`}>
                          {isSender
                            ? "You"
                            : selectedGroup
                            ? msg.senderName || "User"
                        ...
                        <div className={`px-4 py-2 rounded-2xl ${isSender ? "bg-indigo-600 text-white" : "bg-gray-300 text-black"}`}>
                          {editingMessageId === msg._id ? (
                            <div className="flex flex-col gap-2">
                              ...
                              />
                              <button onClick={() => handleUpdateMessage(msg._id)} className="text-xs text-green-300">
                                Save
                              </button>
                            </div>
                          ) : msg.isDeleted ? (
                            ...
                            <div className="flex flex-col gap-1">
                              {msg.fileType?.startsWith("image/") ? (
                                ...
                                <video controls className="max-w-xs rounded-lg">
                                  <source src={`http://localhost:3000${msg.fileUrl}`} type={msg.fileType} />
                                </video>
                              ) : (
                                <a
                                ...
                                >
                                  <span className="text-2xl">
                                    {msg.fileType?.includes("pdf") ? "📄"
                                      ...
                                      : msg.fileType?.includes("zip") || msg.fileType?.includes("rar") ? "🗜️"
                                      : "📎"}
                                  </span>
                            ...
                            <>
                              {msg.message}
                              {msg.isEdited && <span className="text-xs ml-2 opacity-70">(edited)</span>}
                            </>
                          )}
                        </div>
                        ...
                        {isSender && !msg.isDeleted && (
                          <div className="flex gap-3 mt-1">
                            ...
                            <button onClick={() => handleDeleteMessage(msg._id)} className="text-xs text-red-500">
                              Delete
                            </button>
                          </div>
                        )}

                        <p className={`text-[10px] mt-1 ${isSender ? "text-right text-gray-500" : "text-left text-gray-500"}`}>
                          {messageTime}
                        </p>
                      </div>
                    </div>
          ...
          {/* INPUT */}
          <form
            onSubmit={selectedGroup ? handleSendGroupMessage : handleSendMessage}
            className="p-4 bg-white border-t flex gap-3 items-center"
          >
            <input
              type="text"
              placeholder={groupMessagesLoading ? "Loading messages..." : "Type your message..."}
              value={newMessage}
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    if (selectedGroup) handleSendGroupMessage({ preventDefault: () => {} }, file);
